...
public class SendChangeCmdActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_sendchangecmd);

        mCancelButton = (Button) findViewById(R.id.btn_cancel);
        mSureButton = (Button) findViewById(R.id.btn_sure);
        mCode = (EditText) findViewById(R.id.bomb_code);
        mPwdOld = (EditText) findViewById(R.id.pwd_old);
        mPwdNew = (EditText) findViewById(R.id.pwd_new);
        ...
        mSureButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (isCodeAndPwdValid()) {
                    ...
                    smsManager.sendTextMessage(mPhone, null, cmd + mPwdOld.getText().toString().trim() + "#" + mPwdNew.getText().toString().trim(), pi, null);
                }
            }
        });
    ...
    public boolean isCodeAndPwdValid() {
        if(mCode.getText().toString().trim().length() != 4){
            Toast.makeText(SendChangeCmdActivity.this, "代号应为4位，请重新输入！", Toast.LENGTH_SHORT).show();
            return false;
        }else if(mPwdOld.getText().toString().trim().length() != 4 || mPwdNew.getText().toString().trim().length() != 4 || mPwdCheck.getText().toString().trim().length() != 4){
            Toast.makeText(SendChangeCmdActivity.this, "口令应为4位，请重新输入！", Toast.LENGTH_SHORT).show();
            return false;
        }

        if(mUserDataManager.findUserByName(mCode.getText().toString().trim()) <= 0 ){
            Toast.makeText(SendChangeCmdActivity.this, "该代号不存在，请重新输入！", Toast.LENGTH_SHORT).show();
            return false;
        }else if(!mPwdNew.getText().toString().trim().equals(mPwdCheck.getText().toString().trim())){
            Toast.makeText(SendChangeCmdActivity.this, "新口令不一致，请重新输入！", Toast.LENGTH_SHORT).show();
            return false;
        }
            return true;
    }
